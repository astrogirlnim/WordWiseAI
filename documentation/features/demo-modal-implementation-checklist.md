...
### ✅ Phase 4, Step 1: Document Creation & Goal Setting - COMPLETED
- [x] Step 1: Document Creation & Goal Setting
    - [x] **User Flow Distinction Logic**
        - [x] Detect user type (anonymous/demo, new user, existing user) using `useAuth` and `useDemoTourContext`.
        - [x] Branch demo logic/UI based on user type:
    ...
    - [x] **UI Highlighting and Guidance**
        - [x] Implement a spotlight/tooltip on the "Create Document" button in the main UI (shadcn Tooltip or custom overlay).
        - [x] Ensure accessibility (ARIA, keyboard navigation) and responsive design.
        - [x] Add a "Next" action in the demo modal to advance after highlighting.
    - [x] **Document Creation Simulation**
        - [x] For **anonymous/demo mode**:
            - [x] On "Set Writing Goals" action, simulate document creation:
                - [x] Create a temporary in-memory document (not persisted to Firestore).
                - [x] Prepopulate with sample title and content (from `DEMO_SAMPLE_DATA` in `use-demo-tour.ts`).
                - [x] Prepopulate writing goals (from `types/writing-goals.ts` or demo sample).
                - [x] Log all actions for analytics/debugging.
            - [x] Show a success message or badge ("Demo document created!").
        - [x] For **new/existing users**:
            - [x] Only show guidance overlay/modal for document creation and goal setting.
            - [x] Do **not** create or modify any real documents or goals.
            - [x] Optionally, provide a "Learn More" link to documentation.
    - [x] **Writing Goals UI**
        - [x] For all user types:
            - [x] Guide user to the writing goals UI (modal, sidebar, or overlay).
            - [x] For **anonymous/demo mode**: Prepopulate with sample goals.
            - [x] For **new/existing users**: Only highlight/guide, do not prepopulate.
    - [x] **Sample Data Management**
        - [x] Store all sample document and goal data in a single source (`DEMO_SAMPLE_DATA` in `use-demo-tour.ts`).
        - [x] Ensure no hardcoded strings; all sample data should be referenced from this source.
    - [x] **Logging and Analytics**
        - [x] Add extensive `console.log` statements for every action, user type branch, and UI transition.
        - [x] Track demo step completion, skipped steps, and time spent.
    - [x] **Accessibility and Responsiveness**
        - [x] Ensure all overlays, tooltips, and modals are keyboard accessible.
        - [x] Test on mobile and desktop layouts.
    - [x] **Firebase/Backend Considerations**
        - [x] For demo mode, ensure no writes to Firestore or user profile.
        - [x] For new/existing users, ensure demo actions are read-only and do not affect real data.
        - [x] All demo progress is tracked in `demoProgress` (user profile or localStorage).
