      ...
      new Plugin({
        ...
        state: {
          ...
          apply: (tr: Transaction, pluginState: { decorations: DecorationSet }, oldState: EditorState, newState: EditorState) => {
            ...
            // Enhanced validation for each error before creating decorations
            ...
            for (let i = 0; i < newErrors.length; i++) {
              ...
              if (isValidError) {
                // Verify the text at these positions matches the error
                ...
                if (actualText === error.error) {
                  console.log(`[GrammarExtension] BUGFIX: ✓ Text match confirmed for error ${error.id}`);
                  validErrors.push(error);
                } else {
                  console.warn(`[GrammarExtension] BUGFIX: ⚠️ Text mismatch for error ${error.id} - skipping decoration`);
                }
              } else {
