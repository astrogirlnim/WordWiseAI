import type {
  ...
  ToneAlignmentScore,
  ToneRecommendation,
} from '@/types/tone-alignment'
import type { WritingGoals } from '@/types/writing-goals'

export function generateToneAlignmentReport(
  content: string,
  goals: WritingGoals,
): ToneAlignmentReport {
  if (!content.trim()) {
  ...
  const hasFormalWords =
    /\b(therefore|furthermore|consequently|moreover|additionally|specifically)\b/i.test(
      content,
    )
  const hasCasualWords =
    /\b(awesome|cool|great|amazing|super|really|pretty|kinda|gonna|wanna)\b/i.test(
      content,
    )
  const hasMarketingWords =
    /\b(brand|customer|solution|innovative|exclusive|premium|value|benefit|ROI|conversion)\b/i.test(
      content,
    )
  const hasCallToAction =
    /\b(buy|purchase|subscribe|sign up|learn more|get started|contact|download)\b/i.test(
      content,
    )
  ...
  const overallScore = Math.round(
    (audienceScore.alignment +
      formalityScore.alignment +
      domainScore.alignment +
      intentScore.alignment) /
      4,
  )
...
function calculateAudienceAlignment(
  ...
  switch (audience) {
    ...
    case 'stakeholders':
      target = 85
      current = analysis.hasFormalWords
        ? 90
    ...
    case 'industry-experts':
      target = 90
      current =
        analysis.hasFormalWords && analysis.avgWordsPerSentence > 12 ? 85 : 70
      break
  }
...
function calculateFormalityAlignment(
  content: string,
  formality: string,
  analysis: any,
): ToneAlignmentScore {
  ...
  switch (formality) {
    ...
    case 'formal':
      target = 90
      current =
        analysis.hasFormalWords && analysis.avgWordsPerSentence > 15 ? 85 : 65
      break
  }
...
function calculateDomainAlignment(
  ...
  const domainScores = {
    'marketing-copy':
      analysis.hasMarketingWords && analysis.hasCallToAction ? 85 : 60,
...
function getAudienceSuggestion(audience: string): string {
  ...
  return (
    suggestions[audience as keyof typeof suggestions] ||
    'Adjust language for your audience'
  )
}
...
function getAudienceExamples(audience: string): string[] {
  const examples = {
    consumers: [
      ...
      'Add personal benefits and outcomes',
    ],
    stakeholders: ['Include ROI metrics', 'Use strategic business language'],
    'internal-team': [
      "Use 'we' and 'our team'",
      'Include collaborative action items',
    ],
    'industry-experts': [
...
function getFormalitySuggestion(formality: string): string {
  ...
  return (
    suggestions[formality as keyof typeof suggestions] ||
...
function getFormalityExamples(formality: string): string[] {
  const examples = {
    ...
    professional: [
      "Use 'we recommend' instead of 'we think'",
...
function getDomainSuggestion(domain: string): string {
  ...
  return (
    suggestions[domain as keyof typeof suggestions] ||
...
function getIntentSuggestion(intent: string): string {
  ...
  return (
    suggestions[intent as keyof typeof suggestions] || 'Strengthen your intent'
  )
}
